...
def compatibility_ve(install, log_phase, vessels, equipments, port_data):
    ...
    for seq in log_phase.op_ve:
        ...
        for combi in range(len(log_phase.op_ve[seq].ve_combination)):
            ...
            for ves_type in range(nr_diff_ves):  # Agregatte vessel type solutions
                sols_ves.append(tuple([type_of_ves, ves_indexs[ves_type]]))
            ...
            for eq_type in range(nr_diff_equi):  # Agregatte equipment type solutions
                sols_eq.append(tuple([type_of_eq, eq_indexs[eq_type]]))
    ...
    for typ_req in range(len(req_m_ev)):
        ...
        for seq in range(len(log_phase.op_ve)):
            ...
            for combin in range(len(sols_ve_indxs_combs_inseq[seq])):
                ...
                m_ves_ind = int(sols_ve_indxs_combs_inseq[seq][combin][0][0])  # WILL CHANGE WHEN ONLY SUPPOSED VESSEL IS CHECKED !!!!!!!! (no need for last [0] index then!)
                m_eq_ind = int(sols_ve_indxs_combs_inseq[seq][combin][1][0])  # WILL CHANGE WHEN ONLY SUPPOSED VESSEL IS CHECKED !!!!!!!! (no need for last [0] index then!)
