    ...
    for seq in log_phase.op_ve:
        ...
        for combi in range(len(log_phase.op_ve[seq].ve_combination)):
            ...
            for eq_type in range(nr_diff_equi):  # Agregatte equipment type solutions
                ...
                eq_reltd_ves = eq_sol[eq_type]['req_vessel']
                # ves_type_name = ves_sol[eq_reltd_ves]['type']
                # eq_type_name = eq_sol[eq_type]['type']

                sols_ves = tuple(ves_indexs[eq_reltd_ves])
                sols_eq = tuple(eq_indexs[eq_type])
                # sols_ves = [tuple(ves_indexs[eq_reltd_ves]), ves_type_name]
                # sols_eq = [tuple(eq_indexs[eq_type]), eq_type_name]

                sols_ve_indxs_sprt = (sols_ves, sols_eq)  # Agregatte vessel and equipment solutions
            ...
            log_phase.op_ve[seq].sol_combi_combinations[combi] = sols_ve_indxs_combs_incomb
    ...
    for typ_req in range(len(req_m_ev)):
        ...
        for seq in range(len(log_phase.op_ve)):
            ...
            # for m_ves_ind in range(len(log_phase.op_ve[seq].sol_ves)):
                ...
                # vessel:
                m_v_key_type = log_phase.op_ve[seq].sol_combi_ves[combi][0]['type']  # CHANGE WHEN ONLY SUPPOSED VESSEL IS CHECKED !!!!!!!! (no need for [0] index then!)
                m_v_pd = log_phase.op_ve[seq].sol_combi_ves[combi][0]['Series']  # CHANGE WHEN ONLY SUPPOSED VESSEL IS CHECKED !!!!!!!! (no need for [0] index then!)
                m_ves_ind_vec = log_phase.op_ve[seq].sol_combi_ves[combi][0]['indexs']  # CHANGE WHEN ONLY SUPPOSED VESSEL IS CHECKED !!!!!!!! (no need for [0] index then!)
                # equipment:
                m_e_key_type = log_phase.op_ve[seq].sol_combi_eq[combi][0]['type']  # CHANGE WHEN ONLY SUPPOSED VESSEL IS CHECKED !!!!!!!! (no need for [0] index then!)
                m_e_pd = log_phase.op_ve[seq].sol_combi_eq[combi][0]['Series'] # CHANGE WHEN ONLY SUPPOSED VESSEL IS CHECKED !!!!!!!! (no need for [0] index then!)
                m_eq_ind_vec = log_phase.op_ve[seq].sol_combi_eq[combi][0]['indexs']  # CHANGE WHEN ONLY SUPPOSED VESSEL IS CHECKED !!!!!!!! (no need for [0] index then!)


                for ind_ves_indx in range(len(m_ves_ind_vec)):
                    # m_ves_ind = int(m_ves_ind_vec[ind_ves_indx])
                    m_ves_ind = ind_ves_indx
                    for ind_eq_indx in range(len(m_eq_ind_vec)):
                        # m_eq_ind = int(m_eq_ind_vec[ind_eq_indx])
                        m_eq_ind = ind_eq_indx
                           ...
                           for req in range(len(req_m_ev[m_ev_key_req])):
                               ...
                               for ind_rd in range(1,len(m_ev_read)-1,2):
                                   ...
                                   if m_ev_read[ind_rd] == 'plus':
                                       ...
                                       m_v_sol = m_v_pd[ m_v_pd[m_ves_ind][m_ev_read[ind_rd+1]]  >= aux_op ]
