...
def initialize_devices_phase(log_op, vessels, equipments, user_inputs, hydrodynamic_outputs):
    ...
    for index, row in hydro_db.iterrows():
        ...
        if dev_type == 'float WEC' or 'float TEC':
            ...
            phase.op_ve[0].op_sequence[index].extend([ log_op["PosFLTdev"] ])

        elif dev_type == 'fixed WEC' or 'fixed TEC':

             phase.op_ve[0].op_sequence[index].extend([ log_op["PosBFdev"] ])
    ...
    for index, row in hydro_db.iterrows():
        ...
        # initialize an empty operation sequence list for the 'index' element
        phase.op_ve[1].op_sequence[index] = []

        if dev_type == 'float WEC' or 'float TEC':

            phase.op_ve[1].op_sequence[index].extend([ log_op["PosFLTdev"] ])

        elif dev_type == 'fixed WEC' or 'fixed TEC':

             phase.op_ve[1].op_sequence[index].extend([ log_op["PosBFdev"] ])


# CREATE THE INTELIGENCE TO ERASE THE STRATEGY NOT REQUIRED!!!!!!!!
    return phase
